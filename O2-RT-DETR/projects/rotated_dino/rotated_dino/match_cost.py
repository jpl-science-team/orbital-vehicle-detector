...
class ChamferCost(BaseMatchCost):
    """ChamferCost.

    Note: ``bboxes`` in ``InstanceData`` passed in is of format 'cxcywha'
    and its coordinates are unnormalized.

    Args:
        box_format (str, optional): Defaults to 'cxcywha'.
        weight (Union[float, int]): Cost weight. Defaults to 1.
    """
    def __init__(self,
                 box_format: str = 'xywha',
    ...
    def __call__(self,
        ...
        Args:
            pred_instances (:obj:`InstanceData`): ``bboxes`` inside is
                ...
                (cx, cy, w, h, angle).
            gt_instances (:obj:`InstanceData`): ``bboxes`` inside is gt
                bboxes with unnormalized coordinate (cx, cy, w, h, angle).
            img_meta (Optional[dict]): Image information. Defaults to None.
        ...
        corners1 = box2corners(pred_bboxes.unsqueeze(0)).squeeze(0) # (N, 4, 2)
        corners2 = box2corners(gt_bboxes.unsqueeze(0)).squeeze(0)   # (M, 4, 2)

        # Expand for broadcasting: (N, 1, 4, 1, 2) and (1, M, 1, 4, 2)
        corners1_expand = corners1.unsqueeze(1).unsqueeze(3)
        corners2_expand = corners2.unsqueeze(0).unsqueeze(2)

        # L2 distance (Euclidean) between all corner pairs
        dist = torch.norm(corners1_expand - corners2_expand, dim=-1)  # (N, M, 4, 4)

        # Chamfer distance: bidirectional min + mean
        min_dist_pred_to_gt = dist.min(dim=-1)[0].mean(dim=-1)  # (N, M)
        min_dist_gt_to_pred = dist.min(dim=-2)[0].mean(dim=-1)  # (N, M)
        bbox_cost = min_dist_pred_to_gt + min_dist_gt_to_pred
