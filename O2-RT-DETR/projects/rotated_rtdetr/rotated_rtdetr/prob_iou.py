...
if __name__ == '__main__':
    ...
    # 原始尺度：图像 1024x1024
    img_size = 1024.0
    base_pixel = torch.tensor([[512.0, 512.0, 200.0, 100.0, math.pi / 4]])  # 中心，宽高，45度

    # 归一化到 [0,1]
    base_norm = base_pixel.clone()
    base_norm[:, :4] /= img_size      # x,y,w,h ∈ [0,1]

    iou_wrong = probiou(base_norm, base_norm)
    print(f"归一化输入 → ProbIoU = {iou_wrong.item():.6f} (应≈1.0)")

    iou_correct = probiou(base_pixel, base_pixel)
    print(f"原始尺度输入 → ProbIoU = {iou_correct.item():.6f} (应≈1.0)")

    # 测试两个不同尺度但几何相同的框
    box1_pixel = torch.tensor([[0.0, 0.0, 100.0, 50.0, 0.5]])
    box2_pixel = torch.tensor([[0.0, 0.0, 100.0, 50.0, 1.5]])  # 放大2倍
    ...
    print(f"ProbIoU = {iou_scale_wrong.item():.6f} (与上不同！)")
